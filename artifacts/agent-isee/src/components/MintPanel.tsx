      ...
      <div className="flex gap-4 font-mono text-xs">
        ...
        <div className="flex-1 border border-primary/20 bg-black/40 p-3">
          ...
          <div className="text-white font-bold tabular-nums">
            {parseFloat(balance).toFixed(4)} <span className="text-primary/50 font-normal">RITUAL</span>
          </div>
        </div>
      </div>
...
export function MintPanel() {
  const {
    ...
    mint, checkReveal, isConnecting, isOwner,
  } = useBlockchain();
  ...
  return (
    <div className="w-full max-w-md mx-auto p-6 bg-card border border-primary/20 electric-glow rounded-sm relative z-10">
      ...
      {account && (
        <div className="flex items-center justify-between mb-4 px-3 py-2 border border-primary/20 bg-black/40">
          <div className="flex items-center gap-2">
            <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-pulse" />
            <span className="font-mono text-xs text-cyan-300 tracking-widest">
              {shortenAddress(account)}
            </span>
          </div>
          <button
            onClick={disconnectWallet}
            className="font-mono text-[10px] text-primary/40 hover:text-primary/80 uppercase tracking-widest transition-colors"
      ...
      <div className="space-y-3">
        {!account ? (
          <>
            ...
            >
              ADD RITUAL CHAIN TO METAMASK / OKX
            </Button>
          </>
        ) : !isCorrectChain ? (
          <Button
      ...
      {/* Owner-only panel — invisible to all other wallets */}
      {isOwner && <AdminPanel />}
    </div>
  );
}
